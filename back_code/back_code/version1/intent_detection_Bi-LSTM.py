...
class BiLSTM(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers=2, dropout=0.5):
        ...
        self.fc = nn.Linear(hidden_dim * 2, output_dim)  # 双向 LSTM, 需乘 2
...
if __name__ == "__main__":
    ...
    model = BiLSTM(vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, dropout).to(device)
    ...
    df_log.to_csv("BiLSTM_training_log.csv", index=False)
    print("训练数据已保存为 BiLSTM_training_log.csv")
    ...
    plt.tight_layout()
    plt.savefig("bilstm_training_metrics.png")
    plt.show()

    #torch.save(model.state_dict(), 'bilstm_text_classification.pth')
    print("Model saved to 'bilstm_text_classification.pth'")
